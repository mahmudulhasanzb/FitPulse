...
import {
  Calendar, Heart, User, Mail, Clock, AlertTriangle, ChevronRight, Sparkles, FileText,
} from 'lucide-react';
import Image from 'next/image';
import Link from 'next/link';

const StudentOverViewClient = ({ stats }) => {
  const { data: session } = useSession();
  const user = session?.user;
  const appStatus = stats?.application?.status;
    ...
    <div className="flex-1 bg-[#0A0D02] min-h-screen px-6 py-10 md:px-12 md:py-16 text-white overflow-y-auto">
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-5xl mx-auto space-y-10">
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <Link href="/dashboard/student/booked-classes" className="relative group bg-[#13160B] border border-[#1C210E] rounded-3xl p-6 overflow-hidden transition-all duration-300 hover:border-[#D4FF00]/40">
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-[#A4A896]/60 uppercase tracking-wider">Booked Classes</p>
                <h3 className="text-4xl md:text-5xl font-black mt-2 text-white font-mono tracking-tight">{stats?.totalBookings || 0}</h3>
              </div>
          ...
          <Link href="/dashboard/student/favourite-classes" className="relative group bg-[#13160B] border border-[#1C210E] rounded-3xl p-6 overflow-hidden transition-all duration-300 hover:border-[#D4FF00]/40">
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-[#A4A896]/60 uppercase tracking-wider">Favorite Classes</p>
                <h3 className="text-4xl md:text-5xl font-black mt-2 text-white font-mono tracking-tight">{stats?.totalFavorites || 0}</h3>
              </div>
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-5 gap-6">
          <div className="md:col-span-2 bg-[#13160B] border border-[#1C210E] rounded-3xl p-6 flex flex-col items-center text-center space-y-6">
            ...
            <div className="relative w-28 h-28 rounded-full overflow-hidden border-2 border-[#1C210E] p-1 bg-[#14180A]/60 flex items-center justify-center">
              {user?.image ? (
                <div className="w-full h-full rounded-full overflow-hidden relative">
                  <Image src={user.image} fill alt="User avatar" className="object-cover" />
                </div>
              ) : (
            ...
            <div className="space-y-2 w-full">
              <h2 className="text-xl font-bold text-white truncate px-2">{user?.name || 'Guest User'}</h2>
              <div className="flex items-center justify-center gap-1.5 text-xs text-[#A4A896]/70 truncate px-2">
                ...
                <span className="truncate">{user?.email || 'guest@fitpulse.com'}</span>
              </div>
            </div>
            <div className="pt-2 w-full">
              <div className="inline-block text-xs font-black uppercase tracking-widest bg-[#D4FF00]/10 text-[#D4FF00] px-4 py-1.5 rounded-full border border-[#D4FF00]/20 capitalize">{user?.role || 'Student'}</div>
            </div>
          </div>
          ...
          <div className="md:col-span-3 bg-[#13160B] border border-[#1C210E] rounded-3xl p-6 flex flex-col justify-between">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-bold text-[#A4A896]/40 uppercase tracking-widest">Trainer Application</h3>
              </div>
              {!appStatus ? (
                <div className="space-y-4 pt-4">
                  <p className="text-xs text-[#A4A896]/60">You haven't applied to become a trainer yet.</p>
                  <Link href="/dashboard/student/apply-as-trainer" className="inline-flex items-center gap-1.5 text-xs font-bold bg-[#D4FF00] hover:bg-[#c2eb00] text-[#121212] px-4 py-2.5 rounded-full transition-colors uppercase">
                ...
                <div className="space-y-4 pt-4">
                  ...
                  <p className="text-xs md:text-sm text-[#A4A896]/80 leading-relaxed">
                    Your application is being evaluated by our team. This process normally takes 2-3 business days.
                  </p>
                </div>
              ) : appStatus === 'Rejected' ? (
                <div className="space-y-4 pt-4">
                  ...
                  {stats.application?.feedback && (
                    <div className="bg-[#1C1111]/30 border border-[#2F1818] rounded-2xl p-4 space-y-2">
                      <div className="text-[10px] font-black uppercase tracking-widest text-red-400/80">Feedback from Administrator</div>
                      <p className="text-xs text-[#A4A896]/90 leading-relaxed italic">"{stats.application.feedback}"</p>
                    </div>
                  )}
                  <Link href="/dashboard/student/apply-as-trainer" className="inline-flex items-center gap-1.5 text-xs font-bold bg-[#D4FF00] hover:bg-[#c2eb00] text-[#121212] px-4 py-2.5 rounded-full transition-colors uppercase">
                    <FileText className="h-3.5 w-3.5" />
                ...
                <div className="space-y-4 pt-4">
                  <div className="inline-flex items-center gap-2 bg-[#D4FF00]/10 text-[#D4FF00] border border-[#D4FF00]/20 px-3.5 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider">
                    <span>Approved</span>
                  </div>
                  <p className="text-xs text-[#A4A896]/80">Congratulations! You are now a trainer.</p>
                </div>
              ) : null}
            </div>
          </div>
...
export default StudentOverViewClient;
